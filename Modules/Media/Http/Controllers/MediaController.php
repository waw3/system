...
class MediaController extends Controller
{
    ...
    {
        page_title()->setTitle(trans('modules.media::media.menu_name'));

        if ($request->input('media-action') === 'select-files') {
            return view('modules.media::popup');
        }
    ...
    {
        return view('modules.media::popup')->render();
    }
        ...
        switch ($request->input('view_in')) {
            case 'all_media':
                $breadcrumbs = [
                    [
                        'id'   => 0,
                        'name' => trans('modules.media::media.all_media'),
            ...
            case 'trash':
                $breadcrumbs = [
                    [
                        'id'   => 0,
                        'name' => trans('modules.media::media.trash'),
                        'icon' => 'fa fa-trash',
                    ],
                ];
            ...
            case 'recent':
                $breadcrumbs = [
                    [
                        'id'   => 0,
                        'name' => trans('modules.media::media.recent'),
                        'icon' => 'fa fa-clock',
                    ],
                ];
            ...
            case 'favorites':
                $breadcrumbs = [
                    [
                        'id'   => 0,
                        'name' => trans('modules.media::media.favorites'),
                        'icon' => 'fa fa-star',
                    ],
                ];
    ...
    {
        $response = RvMedia::responseError(trans('modules.media::media.invalid_action'));
        ...
        switch ($type) {
            case 'trash':
                ...
                if ($error) {
                    $response = RvMedia::responseError(trans('modules.media::media.trash_error'));
                    break;
                }

                $response = RvMedia::responseSuccess([], trans('modules.media::media.trash_success'));
                break;
            ...
            case 'restore':
                ...
                if ($error) {
                    $response = RvMedia::responseError(trans('modules.media::media.restore_error'));
                    break;
                }

                $response = RvMedia::responseSuccess([], trans('modules.media::media.restore_success'));
                break;
            ...
            case 'make_copy':
                ...
                $response = RvMedia::responseSuccess([], trans('modules.media::media.copy_success'));
                break;
            ...
            case 'delete':
                ...
                $response = RvMedia::responseSuccess([], trans('modules.media::media.delete_success'));
                break;
            case 'favorite':
                ...
                $response = RvMedia::responseSuccess([], trans('modules.media::media.favorite_success'));
                break;
            ...
            case 'remove_favorite':
                ...
                $response = RvMedia::responseSuccess([], trans('modules.media::media.remove_favorite_success'));
                break;
            ...
            case 'rename':
                ...
                if (!empty($error)) {
                    $response = RvMedia::responseError(trans('modules.media::media.rename_error'));
                    break;
                }

                $response = RvMedia::responseSuccess([], trans('modules.media::media.rename_success'));
                break;
            ...
            case 'empty_trash':
                ...
                $response = RvMedia::responseSuccess([], trans('modules.media::media.empty_trash_success'));
                break;
        }
    ...
    protected function copyFile($file, $newFolderId = null)
    {
        ...
        if ($newFolderId == null) {
            ...
            if (!in_array($file->type,
                array_merge(['video', 'youtube'], mconfig('media.config.external_services')))) {
        ...
        if (count($items) == 1 && $items['0']['is_folder'] == 'false') {
            ...
            if (!empty($file) && $file->type != 'video') {
                ...
                if (!Str::contains($file->url, 'https://')) {
                    if (!file_exists($filePath)) {
                        return RvMedia::responseError(trans('modules.media::media.file_not_exists'));
                    }
            ...
            return RvMedia::responseError(trans('modules.media::media.download_file_error'));
        }

        return RvMedia::responseError(trans('modules.media::media.can_not_download_file'));
    }
}
