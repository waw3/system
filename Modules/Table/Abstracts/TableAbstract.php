...
abstract class TableAbstract extends DataTable
{
     ...
     * @var string
     */
    protected $view = 'modules.table::table';

    /**
     * @var string
     */
    protected $filterTemplate = 'modules.table::filter';
            ...
            ->parameters([
                ...
                'lengthMenu'   => Arr::sortRecursive([
                    ...
                    array_values(array_unique(array_merge([10, 30, 50],
                        [$this->pageLength, trans('modules.base::tables.all')]))),
                ]),
                ...
                'language'     => [
                    ...
                    'emptyTable'        => trans('modules.base::tables.no_data'),
                    'info'              => view('modules.table::table-info')->render(),
                    'infoEmpty'         => trans('modules.base::tables.no_record'),
                    'lengthMenu'        => Html::tag('span', '_MENU_', ['class' => 'dt-length-style'])->toHtml(),
                    'search'            => '',
                    'searchPlaceholder' => trans('modules.table::general.search'),
                    'zeroRecords'       => trans('modules.base::tables.no_record'),
                    'processing'        => Html::image(url('vendor/core/images/loading-spinner-blue.gif')),
                    'paginate'          => [
                        'next'     => trans('pagination.next'),
                        'previous' => trans('pagination.previous'),
                    ],
                    'infoFiltered'      => trans('modules.table::general.filtered'),
                ],
    ...
    public function getActionsButton(): array
    {
        ...
        return [
            [
                'extend'  => 'collection',
                'text'    => '<span>' . trans('modules.base::forms.actions') . ' <span class="caret"></span></span>',
    ...
    public function bulkActions(): array
    {
        ...
        if ($this->getBulkChanges()) {
            $actions['bulk-change'] = view('modules.table::bulk-changes', [
    ...
    public function applyFilterCondition($query, string $key, string $operator, ?string $value)
    {
        ...
        switch ($key) {
            case 'created_at':
            case 'updated_at':
                $value = Carbon::createFromFormat(mconfig('base.config.date_format.date'), $value)->toDateString();
    ...
    public function getValueInput(?string $title, ?string $value, ?string $type, $data = null): array
    {
        ...
        $attributes = [
            ...
            'placeholder'  => trans('modules.table::general.value'),
            'autocomplete' => 'off',
        ];
        ...
        switch ($type) {
            case 'select':
                ...
                $attributes['placeholder'] = trans('modules.table::general.select_option');
            ...
            case 'date':
                ...
                $attributes['data-date-format'] = mconfig('base.config.date_format.js.date');
                $html = view('modules.table::partials.date-field', [
    ...
    public function prepareBulkChangeValue(string $key, ?string $value): string
    {
        ...
        switch ($key) {
            case 'created_at':
            case 'updated_at':
                $value = Carbon::createFromFormat(mconfig('base.config.date_format.date'), $value)
                    ->toDateTimeString();
                break;
        }
    ...
    {
        return [
            0 => trans('modules.base::base.no'),
            1 => trans('modules.base::base.yes'),
        ];
    }
    ...
    protected function addCreateButton(string $url, $permission = null, array $buttons = []): array
    {
        if (!$permission || Auth::user()->hasPermission($permission)) {
            ...
            $buttons['create'] = [
                'link' => $url,
                'text' => view('modules.base::elements.tables.actions.create')->render(),
            ];
        }
    ...
    {
        if (!$permission || Auth::user()->hasPermission($permission)) {
            $actions['delete-many'] = view('modules.table::partials.delete', [
                'href'       => $url,
