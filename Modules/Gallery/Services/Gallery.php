...
class Gallery
{
    ...
    public function registerModule($model)
    {
        ...
        config([
            'modules.gallery.config.supported' => array_merge(config('modules.gallery.config.supported', []), $model),
        ]);
    ...
    {
        if ($data != false && in_array(get_class($data), mconfig('gallery.config.supported', []))) {
    ...
    {
        if (in_array(get_class($data), mconfig('gallery.config.supported', []))) {
            $this->galleryMetaRepository->deleteBy([
                'reference_id'   => $data->id,
                'reference_type' => get_class($data),
            ]);
