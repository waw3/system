<?php namespace Modules\DevTool\Services;

use DB, Schema, Config, File;
...
class Seed
{
     ...
     *
     * @access public
     * @param Filesystem $filesystem (default: null)
     * @param Composer $composer (default: null)
     * @return void
     */
    ...
    public function generateSeed($table, $prefix=null, $suffix=null, $database = null, $max = 0, $chunkSize = 0, $exclude = null, $prerunEvent = null, $postrunEvent = null, $dumpAuto = true, $indexed = true, $orderBy = null, $direction = 'ASC')
    {
        ...
        File::put($seedsPath, $seedContent);
    ...
    {
        return base_path(mconfig('devtool.config.seed.path'));
    }
    ...
    {
        return __DIR__ . '/../Resources/stubs/seed';
    }
    ...
    {
        $chunkSize = $chunkSize ?: mconfig('devtool.config.seed.chunk_size');
        $inserts = '';
        ...
        foreach ($chunks as $chunk) {
            ...
            $inserts .= sprintf("DB::table('%s')->insert(%s);", $table, $this->prettifyArray($chunk, $indexed));
        }
    ...
    {
        $content = ($indexed) ? var_export($array, true) : preg_replace("/[0-9]+ \=\>/i", '', var_export($array, true));
        ...
        for ($i = 1; $i < count($lines); $i++) {
            ...
            if ($inString === false) {
                for ($j = 0; $j < $tabCount; $j++) {
                    $lines[$i] = substr_replace($lines[$i], TAB, 0, 0);
                }
            }
    ...
    public function cleanSection()
    {
        ...
        $content = preg_replace("/(\#seedgen_start.+?)\#seedgen_end/us", "#seedgen_start" . NL . TAB . TAB ."#seedgen_end", $content);
        return File::put($databaseSeederPath, $content) !== false;
        return false;
    }
    ...
    public function updateDatabaseSeederRunMethod($className)
    {
        ...
        if (strpos($content, "\$this->call({$className}::class)") === false) {
            if (strpos($content, '#seedgen_start') && strpos($content, '#seedgen_end') && strpos($content, '#seedgen_start') < strpos($content, '#seedgen_end')) {
                $content = preg_replace("/(\#seedgen_start.+?)(\#seedgen_end)/us", "$1\$this->call({$className}::class);" . NL . TAB . TAB ."$2", $content);
            } else {
                $content = preg_replace("/(run\(\).+?)}/us", "$1" . TAB . "\$this->call({$className}::class);" . NL . TAB, $content);
            }
        }
