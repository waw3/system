        ...
        <li class="dd-item dd3-item @if ($row->reference_id > 0) post-item @endif" data-reference-type="{{ $row->reference_type }}"
            ...
            <div class="item-details">
                <label class="pad-bot-5">
                    <span class="text pad-top-5 dis-inline-block" data-update="title">{{ trans('modules.menu::menu.title') }}</span>
                    <input type="text" name="title" value="{{ $row->title }}"
                           data-old="{{ $row->title }}">
                </label>
                @if (!$row->reference_id)
                    <label class="pad-bot-5 dis-inline-block">
                        <span class="text pad-top-5" data-update="custom-url">{{ trans('modules.menu::menu.url') }}</span>
                        <input type="text" name="custom-url" value="{{ str_replace(url(''), '', $row->url) }}" data-old="{{ str_replace(url(''), '', $row->url) }}">
                    </label>
                @endif
                <label class="pad-bot-5 dis-inline-block">
                    <span class="text pad-top-5" data-update="icon-font">{{ trans('modules.menu::menu.icon') }}</span>
                    <input type="text" name="icon-font" value="{{ $row->icon_font }}" data-old="{{ $row->icon_font }}">
                </label>
                <label class="pad-bot-10">
                    <span class="text pad-top-5 dis-inline-block">{{ trans('modules.menu::menu.css_class') }}</span>
                    <input type="text" name="class" value="{{ $row->css_class }}" data-old="{{ $row->css_class }}">
                </label>
                <label class="pad-bot-10">
                    <span class="text pad-top-5 dis-inline-block">{{ trans('modules.menu::menu.target') }}</span>
                    <div style="width: 228px; display: inline-block">
                        <div class="ui-select-wrapper">
                            <select name="target" class="ui-select" id="target" data-old="{{ $row->target }}">
                                <option value="_self" @if ($row->target == '_self') selected="selected" @endif>{{ trans('modules.menu::menu.self_open_link') }}
                                </option>
                                <option value="_blank" @if ($row->target == '_blank') selected="selected" @endif>{{ trans('modules.menu::menu.blank_open_link') }}
                                </option>
                            </select>
                ...
                <div class="text-right" style="margin-top: 5px">
                    <a href="#" class="btn btn-danger btn-remove btn-sm">{{ trans('modules.menu::menu.remove') }}</a>
                    <a href="#" class="btn btn-primary btn-cancel btn-sm">{{ trans('modules.menu::menu.cancel') }}</a>
                </div>
            </div>
            <div class="clearfix"></div>
            @if ($row->has_child)
