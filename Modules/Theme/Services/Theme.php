...
use Language, SeoHelper, WidgetGroup, Closure, File, Exception;
...
class Theme implements ThemeContract
{
     ...
     * @return void
     */
    public function __construct(Repository $config, Dispatcher $events, Factory $view, Asset $asset, Filesystem $files, Breadcrumb $breadcrumb)
    {
        $this->config = $config;
        $this->events = $events;
        $this->view = $view;
    ...
    {
        return is_dir(themes_path($theme));
    }
    ...
    public function getConfig(?string $key = null)
    {
        // Main package config.
        if (!$this->themeConfig) {
            $this->themeConfig = $this->config->get('modules.theme.config');
        }
    ...
    public function theme(?string $theme = null)
    {
        ...
        $this->asset->addPath(mconfig('theme.config.themeDir') . '/' . $this->theme . '/' . $this->getConfig('containerDir.asset'));
    ...
    public function render($statusCode = 200)
    {
        ...
        $content->withHeaders([
            'Author'      => 'William Whitmire (krackenlabs@gmail.com)',
            'Author-Team' => 'https://krackenlabs.com',
            'CMS'         => 'kracken CMS',
    ...
    {
        return $this->view->make('modules.theme::partials.header')->render();
    }
    ...
    {
        return $this->view->make('modules.theme::partials.footer')->render();
    }
    ...
    {
        return File::requireOnce(__DIR__ . '/../Routes/public.php');
    }
