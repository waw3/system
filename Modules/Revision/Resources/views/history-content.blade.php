<div class="tab-pane" id="tab_history">
    <div class="form-group" style="min-height: 400px;">
        <table class="table table-bordered table-striped" id="table">
            <thead>
                <tr>
                    <th>{{ trans('modules.base::tables.author') }}</th>
                    <th>{{ trans('modules.base::tables.column') }}</th>
                    <th>{{ trans('modules.base::tables.origin') }}</th>
                    <th>{{ trans('modules.base::tables.after_change') }}</th>
                ...
                @if ($model->revisionHistory !== null && count($model->revisionHistory)>0)
                    @foreach($model->revisionHistory as $history)
                        <tr>
                            ...
                            <td style="min-width: 145px;">{{ date_from_database($history->created_at, mconfig('base.config.date_format.date_time')) }}</td>
                        </tr>
                    @endforeach
                @else
                    <tr class="text-center">
                        <td colspan="5">{{ trans('modules.base::tables.no_record') }}</td>
                    </tr>
                @endif
            </tbody>
        </table>
