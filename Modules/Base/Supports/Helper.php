...
class Helper
{
    ...
    public static function executeCommand(string $command, array $parameters = [], $outputBuffer = null): bool
    {
        if (!function_exists('proc_open')) {
            if (config('app.debug') && mconfig('base.config.can_execute_command')) {
                throw new Exception(trans('modules.base::base.proc_close_disabled_error'));
            }
            return false;
        }

        if (mconfig('base.config.can_execute_command')) {
