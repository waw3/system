@if (!(isset($attributes['without-buttons']) && $attributes['without-buttons'] == true))
    <div style="height: 34px;">
        ...
        <span class="editor-action-item action-show-hide-editor">
            <button class="btn btn-primary show-hide-editor-btn" type="button" data-result="{{ $result }}">{{ trans('modules.base::forms.show_hide_editor') }}</button>
        </span>
        <span class="editor-action-item">
            <a href="#" class="btn_gallery btn btn-primary"
               data-result="{{ $result }}"
               data-multiple="true"
               data-action="media-insert-{{ setting('rich_editor', mconfig('base.config.editor.primary')) }}">
                <i class="far fa-image"></i> {{ trans('modules.media::media.add') }}
            </a>
        </span>
        @if (isset($attributes['with-short-code']) && $attributes['with-short-code'] == true && function_exists('shortcode'))
            <span class="editor-action-item list-shortcode-items">
                <div class="dropdown">
                    <button class="btn btn-primary dropdown-toggle add_shortcode_btn_trigger" data-result="{{ $result }}" type="button" data-toggle="dropdown"><i class="fa fa-code"></i> {{ trans('modules.base::forms.short_code') }}
                    </button>
                    ...
                    <div class="modal-dialog modal-md">
                        <div class="modal-content">
                            <div class="modal-header bg-primary">
                                <h4 class="modal-title"><i class="til_img"></i><strong>{{ trans('modules.base::forms.add_short_code') }}</strong></h4>
                            ...
                            <div class="modal-footer">
                                <button class="float-left btn btn-secondary" data-dismiss="modal">{{ trans('modules.base::tables.cancel') }}</button>
                                <a class="float-right btn btn-primary add_short_code_btn" href="#">{{ trans('modules.base::forms.add') }}</a>
                            </div>
                        </div>
                    </div>
                </div>
...
@if (setting('rich_editor', mconfig('base.config.editor.primary')) === 'tinymce')
    @push('footer')
        <script>
