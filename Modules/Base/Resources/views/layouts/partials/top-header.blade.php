    ...
    <div class="page-header-inner col-12">
            ...
            <div class="page-logo">
                <a href="{{ route('dashboard.index') }}">
                    <img src="{{ setting('admin_logo') ? get_image_url(setting('admin_logo')) : url(mconfig('base.config.logo')) }}" alt="logo" class="logo-default" />
                </a>
                @auth
            ...
            <div class="top-menu">
                <ul class="nav navbar-nav float-right">
                    @auth
                        @if (mconfig('base.config.admin_dir') != '')
                            <li class="dropdown">
                                <a class="dropdown-toggle dropdown-header-name" style="padding-right: 10px" href="{{ url('/') }}" target="_blank"><i class="fa fa-globe"></i> <span class="d-none d-sm-inline">{{ trans('modules.base::layouts.view_website') }}</span> </a>
                            </li>
                        @endif
                        @if (Auth::check())
                        ...
                        @if (isset($themes) && setting('enable_change_admin_theme') != false)
                            <li class="dropdown">
                                <a href="javascript:;" class="dropdown-toggle dropdown-header-name" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
                                    <span>{{ trans('modules.base::layouts.theme') }}</span>
                        ...
                        <li class="dropdown dropdown-user">
                            ...
                            <ul class="dropdown-menu">
                                <li><a href="{{ route('user.profile.view', Auth::user()->getKey()) }}"><i class="icon-user"></i> {{ trans('modules.base::layouts.profile') }}</a></li>
                                <li><a href="{{ route('access.logout') }}" class="btn-logout"><i class="icon-key"></i> {{ trans('modules.base::layouts.logout') }}</a></li>
                            </ul>
                        </li>
                    @endauth
                </ul>
