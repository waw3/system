...
class Handler extends ExceptionHandler
{
    ...
    public function render($request, Throwable $ex)
    {
        if ($ex instanceof PostTooLargeException) {
            return RvMedia::responseError(trans('modules.media::media.upload_failed', [
    ...
    protected function handleResponseData($code, $request)
    {
        if ($request->expectsJson()) {
            ...
            if ($code == 401) {
                return (new BaseHttpResponse)
                    ->setError()
                    ->setMessage(trans('modules.acl::permissions.access_denied_message'))
                    ->setCode($code)
                    ->toResponse($request);
            }
        ...
        if ($request->is(mconfig('base.config.admin_dir') . '/*') || $request->is(mconfig('base.config.admin_dir'))) {
            return response()->view('modules.base::errors.' . $code, [], $code);
        }
    ...
    public function report(Throwable $exception)
    {
        if ($this->shouldReport($exception) && !$this->isExceptionFromBot()) {
            if (!app()->isLocal() && !app()->runningInConsole()) {
                ...
                if (mconfig('base.config.error_reporting.via_slack',
                        false) == true && !$exception instanceof OAuthServerException) {
                    config()->set([
                        'logging.channels.slack.username' => 'CMS BOT',
    ...
    {
        $ignored_bots = mconfig('base.config.error_reporting.ignored_bots', []);
