...
class LanguageManager
{
    ...
    {
        return setting('language_hide_default', mconfig('language.config.hideDefaultLocaleInURL'));
    }
    ...
    {
        return apply_filters(LANGUAGE_FILTER_MODEL_USING_MULTI_LANGUAGE, mconfig('language.config.supported', []));
    }
    ...
    public function registerModule($model)
    {
        ...
        config([
            'modules.language.config.supported' => array_merge(mconfig('language.config.supported', []), $model),
        ]);
    ...
    {
        return mconfig('language.config.useAcceptLanguageHeader');
    }
}
