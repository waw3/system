<li class="dropdown dropdown-extended dropdown-inbox" id="header_inbox_bar">
    ...
    <ul class="dropdown-menu dropdown-menu-right">
        <li class="external">
            <h3>{!! trans('modules.plugins.contact::contact.new_msg_notice', ['count' => count($contacts)]) !!}</h3>
            <a href="{{ route('contacts.index') }}">{{ trans('modules.plugins.contact::contact.view_all') }}</a>
        </li>
        <li>
                ...
                @foreach($contacts as $contact)
                    <li>
                        <a href="{{ route('contacts.edit', $contact->id) }}">
                            <span class="photo">
                                <img src="{{ \Modules\Base\Supports\Gravatar::image($contact->email) }}" class="rounded-circle" alt="{{ $contact->name }}">
                            </span>
