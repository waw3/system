...
namespace Modules\Plugins\PostScheduler\Providers;
...
class HookServiceProvider extends ServiceProvider
{
    ...
    {
        $publishDate = now(config('app.timezone'))->format(mconfig('base.config.date_format.date'));
        $publishTime = now(config('app.timezone'))->format('G:i');

        $args = func_get_args();
        $data = $args[0];
        if ($data && $data->id) {
            $publishDate = date_from_database($data->created_at, mconfig('base.config.date_format.date'));
            $publishTime = date_from_database($data->created_at, 'G:i');
        }

        return view('modules.plugins.post-scheduler::publish-box', compact('publishDate', 'publishTime', 'data'))->render();
    }
    ...
    public function saveSchedulerData($screen, $request, $object)
    {
        if (PostScheduler::isSupportedModule(get_class($object))) {
            ...
            if (!empty($publishDate)) {
                $object->created_at = Carbon::createFromFormat(mconfig('base.config.date_format.date') . ' G:i', $publishDate . ' ' . $publishTime)->toDateTimeString();
                $object->save();
            }
        }
    }
