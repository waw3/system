...
<div>
                ...
                @foreach ($value as $key => $item)
                    <div class="col-md-2 col-sm-3 col-4 photo-gallery-item" data-id="{{ $key }}" data-img="{{ Arr::get($item, 'img') }}" data-description="{{ Arr::get($item, 'description') }}">
                        <div class="gallery_image_wrapper">
                            <img src="{{ get_image_url(Arr::get($item, 'img'), 'thumb') }}" alt="{{ trans('modules.plugins.gallery::gallery.item') }}">
                        </div>
                    </div>
                @endforeach
            @endif
    ...
    <div class="form-group">
        <a href="#" class="btn_select_gallery">{{ trans('modules.plugins.gallery::gallery.select_image') }}</a>&nbsp;
        <a href="#" class="text-danger reset-gallery @if (empty($value)) hidden @endif">{{ trans('modules.plugins.gallery::gallery.reset') }}</a>
    </div>
</div>
    ...
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header bg-danger">
                <h4 class="modal-title"><i class="til_img"></i><strong>{{ trans('modules.plugins.gallery::gallery.update_photo_description') }}</strong></h4>
                <button type="button" class="close" data-dismiss="modal" aria-hidden="true">&times;</button>
            </div>

            <div class="modal-body with-padding">
                <p><input type="text" class="form-control" id="gallery-item-description" placeholder="{{ trans('modules.plugins.gallery::gallery.update_photo_description_placeholder') }}"></p>
            </div>

            <div class="modal-footer">
                <button class="float-left btn btn-danger" id="delete-gallery-item" href="#">{{ trans('modules.plugins.gallery::gallery.delete_photo') }}</button>
                <button class="float-right btn btn-secondary" data-dismiss="modal">{{ trans('modules.base::forms.cancel') }}</button>
                <button class="float-right btn btn-primary" id="update-gallery-item">{{ trans('modules.base::forms.update') }}</button>
            </div>
        </div>
    </div>
</div>
