...
class Gallery
{
    ...
    public function registerModule($model)
    {
        ...
        config([
            'modules.plugins.gallery.general.supported' => array_merge(config('modules.plugins.gallery.general.supported', []), $model),
        ]);
    ...
    {
        if ($data != false && in_array(get_class($data), config('modules.plugins.gallery.general.supported', []))) {
    ...
    {
        if (in_array(get_class($data), config('modules.plugins.gallery.general.supported', []))) {
            $this->galleryMetaRepository->deleteBy([
                'reference_id'   => $data->id,
                'reference_type' => get_class($data),
            ]);
