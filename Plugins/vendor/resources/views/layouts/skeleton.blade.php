...
<head>
  ...
  <script type="text/javascript">
      window.trans = JSON.parse('{!! addslashes(json_encode(trans('modules.plugins.vendor::dashboard'))) !!}');
      var BotbleVariables = BotbleVariables || {};
      BotbleVariables.languages = {
        tables: {!! json_encode(trans('modules.base::tables'), JSON_HEX_APOS) !!},
        notices_msg: {!! json_encode(trans('modules.base::notices'), JSON_HEX_APOS) !!},
        pagination: {!! json_encode(trans('pagination'), JSON_HEX_APOS) !!},
        system: {
          'character_remain': '{{ trans('modules.base::forms.character_remain') }}'
        }
      };
  </script>
</head>
<body>
  ...
  <div id="app">
    @include('modules.plugins.vendor::components.header')
    <main class="pv3 pv4-ns">
        @if (auth('vendor')->check() && !auth('vendor')->user()->canPost())
            <div class="container">
                <div class="alert alert-warning">{{ trans('modules.plugins.vendor::package.add_credit_warning') }}
                    <a href="{{ route('public.vendor.packages') }}">{{ trans('modules.plugins.vendor::package.add_credit') }}</a></div>
            </div>
            <br>
        @endif
      @yield('content')
