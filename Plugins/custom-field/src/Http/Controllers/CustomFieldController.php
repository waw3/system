...
namespace Modules\Plugins\CustomField\Http\Controllers;

use Assets;
use Modules\Base\Forms\FormBuilder;
use Modules\Base\Http\Controllers\BaseController;
use Modules\Base\Http\Responses\BaseHttpResponse;
use Modules\Plugins\CustomField\Actions\CreateCustomFieldAction;
use Modules\Plugins\CustomField\Actions\DeleteCustomFieldAction;
use Modules\Plugins\CustomField\Actions\ExportCustomFieldsAction;
use Modules\Plugins\CustomField\Actions\ImportCustomFieldsAction;
use Modules\Plugins\CustomField\Actions\UpdateCustomFieldAction;
use Modules\Plugins\CustomField\Forms\CustomFieldForm;
use Modules\Plugins\CustomField\Tables\CustomFieldTable;
use Modules\Plugins\CustomField\Http\Requests\CreateFieldGroupRequest;
use Modules\Plugins\CustomField\Http\Requests\UpdateFieldGroupRequest;
...
class CustomFieldController extends BaseController
{
    ...
    {
        page_title()->setTitle(trans('modules.plugins.custom-field::base.page_title'));
    ...
    {
        page_title()->setTitle(trans('modules.plugins.custom-field::base.form.create_field_group'));
    ...
    public function destroy($id, BaseHttpResponse $response, DeleteCustomFieldAction $action)
    {
        try {
            $action->run($id);
            return $response->setMessage(trans('modules.plugins.custom-field::field-groups.deleted'));
        } catch (Exception $ex) {
            return $response
                ->setError()
                ->setMessage(trans('modules.plugins.custom-field::field-groups.cannot_delete'));
        }
    }
    ...
    public function deletes(Request $request, BaseHttpResponse $response, DeleteCustomFieldAction $action)
    {
        ...
        if (empty($ids)) {
            return $response
                ->setError()
                ->setMessage(trans('modules.plugins.custom-field::field-groups.notices.no_select'));
        }
        ...
        return $response->setMessage(trans('modules.plugins.custom-field::field-groups.field_group_deleted'));
    }
