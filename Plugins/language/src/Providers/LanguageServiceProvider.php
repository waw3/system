...
use Modules\Plugins\Language\Facades\LanguageFacade;
use Modules\Plugins\Language\Http\Middleware\LocaleSessionRedirect;
use Modules\Plugins\Language\Http\Middleware\LocalizationRedirectFilter;
use Modules\Plugins\Language\Http\Middleware\LocalizationRoutes;
use Modules\Plugins\Language\Models\Language as LanguageModel;
use Modules\Plugins\Language\Models\LanguageMeta;
use Modules\Plugins\Language\Repositories\Caches\LanguageMetaCacheDecorator;
use Modules\Plugins\Language\Repositories\Eloquent\LanguageMetaRepository;
...
use Modules\Plugins\Language\Repositories\Caches\LanguageCacheDecorator;
use Modules\Plugins\Language\Repositories\Eloquent\LanguageRepository;
...
class LanguageServiceProvider extends ServiceProvider
{
    ...
    {
        $this->setNamespace('modules.plugins.language')
        ...
        if ($this->app->runningInConsole()) {
            ...
            Event::listen(RouteMatched::class, function () {
                dashboard_menu()->registerItem([
                    ...
                    'name'        => 'modules.plugins.language::language.menu',
                    'icon'        => null,
    ...
    public function addLanguageBox($priority, $object)
    {
        if (!empty($object) && in_array(get_class($object), Language::supportedModels())) {
            add_meta_box('language_wrap', trans('modules.plugins.language::language.name'), [$this, 'languageMetaField'],
    ...
    public function addLanguageMetaBoxForThemeOptionsAndWidgets($data, $screen)
    {
        ...
        return $data . view('modules.plugins.language::partials.admin-list-language-chooser', compact('route'))->render();
    }
    ...
    public function languageMetaField()
    {
        ...
        return view('modules.plugins.language::language-box',
    ...
    public function addCurrentLanguageEditingAlert($screen, $request, $data = null)
    {
        if ($data && in_array(get_class($data), Language::supportedModels())) {
            ...
            echo view('modules.plugins.language::partials.notification', compact('language'))->render();
        }
        echo null;
    }
            ...
            return $data->addColumn('language', function ($item) use ($model, $route) {
                ...
                foreach ($languages as $language) {
                    if ($language->lang_code == $currentLanguage) {
                        $data .= view('modules.plugins.language::partials.status.active', compact('route', 'item'))->render();
                    } else {
                        $added = false;
                        if (!empty($relatedLanguages)) {
                            foreach ($relatedLanguages as $key => $relatedLanguage) {
                                if ($key == $language->lang_code) {
                                    $data .= view('modules.plugins.language::partials.status.edit',
                        ...
                        if (!$added) {
                            $data .= view('modules.plugins.language::partials.status.plus',
                                compact('route', 'item', 'language'))->render();
                        }
                    }
                }

                return view('modules.plugins.language::partials.language-column', compact('data'))->render();
            });
        }
    ...
    public function languageSwitcher($options = [])
    {
        ...
        return view('modules.plugins.language::partials.switcher', compact('options', 'supportedLocales'))->render();
    }
    ...
    public function addLanguageSwitcherToTable($buttons, $screen)
    {
        if (in_array($screen, Language::supportedModels())) {
            ...
            $languageButtons[] = [
                ...
                'text'      => Html::tag('span', trans('modules.plugins.language::language.show_all'),
            ...
            $language = [
                'language' => [
                    ...
                    'text'    => $flag . Html::tag('span',
                            ' ' . trans('modules.plugins.language::language.change_language') . ' ' . Html::tag('span', '',
    ...
    {
        return $alert . view('modules.plugins.language::partials.admin-language-switcher')->render();
    }
    ...
    public function addLanguageRefLangTags($header)
    {
        ...
        return $header . view('modules.plugins.language::partials.hreflang', compact('supportedLocales'))->render();
    }
}
