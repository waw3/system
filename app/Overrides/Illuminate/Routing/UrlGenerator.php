...
class UrlGenerator extends BaseUrlGenerator
{
    ...
    {
        if (! mconfig('core.config.route.trailing_slash')) {
        ...
        return $this->format(
                $root, '/'.trim($path.'/'.$tail, '/')
            ).(mconfig('core.config.route.trailing_slash') ? '/' : '').$query;
    }
    ...
    {
        if (mconfig('core.config.route.trailing_slash') && ! $this->routeGenerator) {
    ...
    public function toRoute($route, $parameters, $absolute)
    {
        ...
        if (mconfig('core.config.route.locale_prefix') && in_array('locale', $route->parameterNames()) && ! isset($parameters['locale'])) {
